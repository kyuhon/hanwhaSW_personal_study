...
//    @GetMapping("/first/regist")
    ...
    public String registMenu(HttpServletRequest request, Model model){
        ...
        /* 설명. Service 계층의 메소드를 호출하며 비즈니스 로직(insert 트랜잭션)을 성공하고 들어왔다는 가정 */

        /* 설명. insert 성공 메시지와 함께 성공 페이지로 이동 */
        String message = name + "을(를) 신규 메뉴 목록의" + categoryCode + "빈 카테고리에 "
    ...
    /* 설명.
    *   request의 parameter로 넘어오는 값들의 키값과 일치하는 변수명(어노테이션 생략가능)을 작성하고 @RequestParam
    *   어노테이션을 적용하면 알아서 값을 꺼내고 해당 매개변수의 자료형에 맞게 자료형 변환까지 해준다.(물론 가능한 경우)
    *
    *  설명.
    *   1. defaultValue: 사용자의 입력 값이 없거나("") 아니면 request의 parameter 키 값과 일치하지 않는
    *                     매개변수 사용 시 매개변수가 가질 기본 defalut값을 작성한다.
    *   2. name: request parameter의 키 값과 다른 매개변수 명을 사용하고 싶을 떄 request parameter의
    *            키 값을 작성한다.
    * */

    @PostMapping("modify1")
    public String modifyMenu1(Model model, 
                              @RequestParam(name="modifyName") String modifyName, 
