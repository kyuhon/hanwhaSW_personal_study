...
// @Controller
@RestController     // 이 컨트롤러의 모든 핸들러 메소드의 응답은 view resolver를 활용하지 않는다.(SSR에서 뷰를 사용하지 않겠다)
...
//    @ResponseBody       // 핸들러 메소드에 응답이 json 형태로 되며 자바의 모든 타입 반환이 가능해 진다.
    ...
    * */
    ...
    public byte[] getImageWithBytes(@PathVariable String filename) throws IOException {
        Path path = Paths.get("C:/Users/Playdata/Desktop/Personal_study/image/" + filename + ".png");
    ...
    * */
    ...
    public ResponseEntity<Resource> getImageWithResource(@PathVariable String filename)
        ...
        /* 설명. UriResource: 파일 시스템 경로를 Resource 객체로 변환 */
        Path path = Paths.get("C:/Users/Playdata/Desktop/Personal_study/image/" + filename);
