...
public class EntityLifeCycleTest {
    ...
    public void 비영속성_테스트(){
        ...
        Assertions.assertFalse(foundMenu == newMenu); // 동등하지만 동일하지은 않다
    ...
    public void 준영속성_detach_테스트(){
        ...
        Menu foundMenu2 = entityManager.find(Menu.class, 11);   // select
        ...
        *   설명.
        *       준영속을 사용하는 이유
        *       : JPA에서 엔티티를 더 이상 영속성 컨텍스트에서 관리하지 않음을 의미하며, 특정 상황
        *         (flush를 특정시점에 별도로 사용)에서 성능 최적화나
    ...
    public void 병합_merge_수정_테스트(){
        ...
        /* 설명. 결과적으로 준영속 상태였던 것과 DB에서 다시 조회한 것 중에 어떤 것이 살아남았는지
        *   확인하기 위해 다시 find */
